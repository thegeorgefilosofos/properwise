...
export function shortTermYearSummary(stays: TaxStay[], year: number, meta?: PropertyTaxMeta): ShortTermYearSummary {
  ...
  // εφαρμόζεται η τεκμαρτή έκπτωση 5% (άρθρο 39 §3 ΚΦΕ) → φορολογείται το 95%.
  // Προϋπόθεση (κύρωση από 1.7.2027, ν.5222/2025): είσπραξη μέσω τραπέζης· με μετρητά το 100%.
  // Ο ΣΥΝΤΕΛΕΣΤΗΣ ΔΕΝ ΞΑΝΑΓΡΑΦΕΤΑΙ. Εδώ ήταν `? 1 : 0.95`, δηλαδή ΚΑΙ το
  // ποσοστό ΚΑΙ ο κανόνας της τράπεζας, δεύτερη φορά. Η προϋπόθεση της τραπέζης
  // (ν.5222/2025, άρθρο 210) είναι χωριστή διάταξη από την κλίμακα και η κύρωσή
  // της ξεκινά την 1.7.2027 (Α.1187/2026)· την επόμενη φορά η Λογιστική θα
  // ενημερωνόταν και η Πληρότητα όχι, με δύο φόρους για την ίδια χρονιά.
  // ΚΑΙ ΤΟ ΕΤΟΣ ΜΕΤΡΑΕΙ, ΟΧΙ ΜΟΝΟ Ο ΤΡΟΠΟΣ ΕΙΣΠΡΑΞΗΣ.
  //
  // Εδώ καλούσε την ΑΦΡΑΚΤΗ `presumptiveDeductionRate`, ενώ η φραγμένη με το
  // έτος —`presumptiveDeductionRateForYear`— ζει δίπλα της και γράφτηκε ακριβώς
