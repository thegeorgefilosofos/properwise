...
const KNOWN: [number, number][] = [
  ...
  [50000, 9000 + 14000 * 0.45],        // 15300
  [100000, 9000 + 64000 * 0.45],       // 37800
];
...
ok('marginal(35999)=35%', marginalRate(35999) === 0.35);
ok('marginal(36001)=45%', marginalRate(36001) === 0.45);
...
// ── Οι γραμμές UI ΒΓΑΙΝΟΥΝ από τα κλιμάκια, δεν γράφονται δίπλα τους ────────
{
  ...
  ok('η πρώτη γραμμή ξεκινά στο μηδέν', rows[0].range === '0,00 – 12.000,00€');
  ok('η επόμενη ξεκινά ένα ευρώ πιο πάνω', rows[1].range === '12.001,00 – 24.000,00€');
  ok('η τελευταία δεν έχει οροφή', rows[3].range === 'Πάνω από 36.000,00€');
  ok('και κανένα όριο δεν κρατά κενό πριν το σύμβολο',
    rows.every(r => !/[\u0020\u00A0]€/.test(r.range)));
}
