...
// ═══ ΤΡΑΠΕΖΙΚΗ ΕΙΣΠΡΑΞΗ: Ο ΚΑΝΟΝΑΣ ΤΟΥ 2026, ΕΝΑ ΣΗΜΕΙΟ ════════════════════
{
  ...
  // ── Ο ΤΡΟΠΟΣ ΕΙΣΠΡΑΞΗΣ ΜΕΤΡΑΕΙ ΜΟΝΟ ΑΠΟ ΤΟ 2027 ────────────────────────
  // Η προϋπόθεση της τράπεζας θεσπίστηκε με τον ν.5222/2025 (άρθρο 210), ΟΧΙ
  // τον ν.5246/2025· η κύρωση ξεκινά την 1.7.2027 (Α.1187/2026). Άρα για
  // τις χρήσεις 2025 ΚΑΙ 2026 η έκπτωση δίνεται ανεξάρτητα από τον τρόπο· ένα
  // «με μετρητά» εκεί θα αφαιρούσε έκπτωση που ο νόμος ΔΙΝΕΙ ακόμη.
  eq('2025: ο τρόπος είσπραξης δεν μετράει', bankReceiptMatters(2025), false);
  eq('2026: δεν μετράει ακόμη (κύρωση από 1.7.2027)', bankReceiptMatters(2026), false);
  eq('2027: μετράει', bankReceiptMatters(2027), true);
  eq('χωρίς έτος δεν μετράει (τρέχον καθεστώς 2026)', bankReceiptMatters(null), false);
