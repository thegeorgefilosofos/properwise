...
// ── ΤΟ ΜΕΤΡΟ ΑΚΟΛΟΥΘΕΙ ΤΗ ΧΡΟΝΙΑ ΤΟΥ ΝΟΜΟΥ ────────────────────────────────
// ΤΟ ΠΡΩΤΟ ΣΦΑΛΜΑ. Ο κατάλογος δεν είχε έννοια χρόνου. Η ΔΙΟΡΘΩΣΗ ΤΟΥ ΕΙΧΕ
// ΔΕΥΤΕΡΟ: έγραψε ότι το μέτρο λήγει το 2026, ενώ από τον ΕΝΦΙΑ 2027 γίνεται
// πλήρης απαλλαγή με τα ίδια όρια (άρθρο 17 παρ. 3 ν.5219/2025). Ο ιδιοκτήτης
// στο χωριό θα έβλεπε το 2027 ολόκληρο φόρο που δεν χρωστά.
{
  const KEY = 'small_settlement_2026'
  const rd = ENFIA_REDUCTIONS.find(r => r.key === KEY)!
  ok('το μέτρο δεν λήγει, αλλάζει ποσοστό', rd.untilYear == null && rd.pctFrom?.year === 2027 && rd.pctFrom.pct === 100)
  ok('και ξεκινά το 2026', rd.sinceYear === 2026)

  ok('το 2026 δίνει 50%', enfiaReductionPct(KEY, 200000, 2026) === 50)
  ok('από το 2027 δίνει 100%', enfiaReductionPct(KEY, 200000, 2027) === 100 && enfiaReductionPct(KEY, 200000, 2030) === 100)
  ok('το 2025 δεν υπήρχε', enfiaReductionPct(KEY, 200000, 2025) === 0)
  ok('και η οθόνη το ξέρει',
    enfiaReductionInForce(KEY, 2026) && enfiaReductionInForce(KEY, 2027) && !enfiaReductionInForce(KEY, 2025))
  ok('το όριο αξίας κρατά και στην απαλλαγή', enfiaReductionPct(KEY, 400001, 2027) === 0)
  ...
  ok('χωρίς έτος, το μέτρο με χρονιά έναρξης δεν εφαρμόζεται', enfiaReductionPct(KEY, 200000) === 0)
  ...
  // ── ΤΟ ΟΡΙΟ ΑΞΙΑΣ ΤΗΡΕΙΤΑΙ ΑΠΟ ΤΗ ΜΗΧΑΝΗ, ΟΧΙ ΜΟΝΟ ΑΠΟ ΤΗ ΣΗΜΕΙΩΣΗ ──────
  // Δεύτερη φορά το ίδιο σφάλμα στον ίδιο πίνακα: το `pctOver` γράφτηκε επειδή
  // το κατώφλι της ασφαλισμένης κατοικίας ζούσε μόνο στη σημείωση. Ακριβώς
  // δίπλα, το «αξία κατοικίας ≤400.000€» έκανε το ίδιο.
  ok('το μέτρο δηλώνει το ανώτατο όριο αξίας', rd.maxHomeValue === 400000)
  ok('κάτω από το όριο δίνεται', enfiaReductionPct(KEY, 399999, 2026) === 50)
  ok('πάνω από το όριο δεν δίνεται καθόλου', enfiaReductionPct(KEY, 400001, 2026) === 0)
  ok('ακριβώς στο όριο δίνεται', enfiaReductionPct(KEY, 400000, 2026) === 50)
  // ΑΓΝΩΣΤΗ ΑΞΙΑ ΔΕΝ ΤΙΜΩΡΕΙΤΑΙ. Τα υπόλοιπα κριτήρια τα βεβαιώνει ο χρήστης
  // τσεκάροντας το κουτί· να του κοπεί η μείωση επειδή δεν συμπλήρωσε πεδίο
  // θα ήταν σιωπηλή τιμωρία για κενό, όχι εφαρμογή του νόμου.
  ok('χωρίς δηλωμένη αξία, το όριο δεν κρίνει', enfiaReductionPct(KEY, 0, 2026) === 50)
  ok('και η οθόνη λέει το ίδιο',
    enfiaReductionInForce(KEY, 2026, 399999) && !enfiaReductionInForce(KEY, 2026, 400001))
  ...
  const none27 = estimateENFIA({ ...base, reductions: [], year: 2027 })!
  ok('ο ΕΝΦΙΑ του 2026 είναι μειωμένος', y26.annual > 0 && y26.annual < none27.annual)
  ok('και του 2027 μηδενικός', y27.annual === 0)
}

// ── ΚΑΘΕ ΟΡΙΟ ΠΟΥ ΛΕΕΙ Η ΣΗΜΕΙΩΣΗ ΕΧΕΙ ΚΑΙ ΠΕΔΙΟ ΓΙΑ ΤΗ ΜΗΧΑΝΗ ────────────
// ΓΙΑΤΙ ΔΟΜΙΚΟΣ ΚΑΝΟΝΑΣ ΚΑΙ ΟΧΙ ΑΚΟΜΗ ΕΝΑΣ ΕΛΕΓΧΟΣ ΤΙΜΗΣ. Το σφάλμα «ο κανόνας
// ζει στη σημείωση και η μηχανή δεν τον ξέρει» συνέβη ΔΥΟ φορές στον ίδιο
// πίνακα, με δύο χρόνια απόσταση. Ενας έλεγχος για τη συγκεκριμένη τιμή δεν
// εμποδίζει την τρίτη: εμποδίζει μόνο ο κανόνας ότι κάθε σημείωση που δηλώνει
// ανώτατη αξία κατοικίας οφείλει να έχει δίπλα της το `maxHomeValue`.
{
  const CEILING = /αξία κατοικίας ≤([\d.]+)/
  for (const rd of ENFIA_REDUCTIONS) {
    const m = CEILING.exec(rd.note)
    if (!m) continue
    const stated = Number(m[1].replace(/\./g, ''))
    ok(`η «${rd.key}» τηρεί το όριο που λέει η σημείωσή της`, rd.maxHomeValue === stated)
  }
}
