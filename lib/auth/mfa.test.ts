...
// ── Η ΙΔΙΑ ΕΤΥΜΗΓΟΡΙΑ ΦΥΛΑΕΙ ΚΑΙ ΤΗ ΔΙΑΓΡΑΦΗ ΛΟΓΑΡΙΑΣΜΟΥ ──────────────────
// Το app/api/account/delete/route.ts περνά ΑΚΡΙΒΩΣ αυτό το ζεύγος: διακριτικό
// της συνεδρίας και τους παράγοντες του χρήστη. Ο διαμεσολαβητής εξαιρεί το
// /api/**, οπότε αν εδώ έβγαινε λάθος «false», μια συνεδρία aal1 θα διέγραφε
// ολόκληρο τον λογαριασμό. Θετικό = 403 στη διαδρομή· αρνητικό = προχωρά.
ok('συνεδρία aal1 με συσκευή ΔΕΝ διαγράφει λογαριασμό (403)',
  sessionNeedsSecondStep(token({ aal: 'aal1' }), VERIFIED))
ok('συνεδρία aal2 με συσκευή προχωρά τη διαγραφή',
  !sessionNeedsSecondStep(token({ aal: AAL2 }), VERIFIED))
ok('χρήστης χωρίς συσκευή δεν εμποδίζεται από τη διαγραφή',
  !sessionNeedsSecondStep(token({ aal: 'aal1' }), []))
