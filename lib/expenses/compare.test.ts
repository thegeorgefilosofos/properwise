...
{
  // Νερό ανά δίμηνο: Φεβρουάριος, Απρίλιος, Ιούνιος. Με βάση τον
  // Ιούνιο (όπου ήρθε ο λογαριασμός) ο Ιούλιος δεν έχει νερό. Αυτό δεν
  // είναι «σταμάτησε».
  const data = [
    s('2026-02-10', 60, 'water'), s('2026-04-10', 60, 'water'), s('2026-06-10', 60, 'water'),
    s('2026-06-12', 50, 'electricity'), s('2026-07-12', 50, 'electricity'),
  ];
  const c = compareMonth(data, '2026-07', { today: JUL31 });
  const water = c.drivers.find(d => d.slug === 'water')!;
  ok('ο διμηνιαίος λογαριασμός δεν «σταμάτησε»', !!water && !water.vanished);
}

// ═══ ΟΙ ΓΡΑΜΜΕΣ ΑΘΡΟΙΖΟΥΝ ΣΤΗ ΔΙΑΦΟΡΑ ══════════════════════════════════════
{
  const data = [
    s('2026-06-10', 100, 'electricity'), s('2026-06-11', 100, 'water'), s('2026-06-12', 100, 'internet'),
    s('2026-06-13', 100, 'insurance'), s('2026-06-14', 3, 'cleaning'),
    s('2026-07-10', 200, 'electricity'), s('2026-07-11', 50, 'water'), s('2026-07-12', 160, 'internet'),
    s('2026-07-13', 70, 'insurance'),
  ];
  const c = compareMonth(data, '2026-07', { today: JUL31 });
  const sum = c.drivers.reduce((t, d) => t + d.diff, 0) + c.rest;
  ok('οδηγοί και «λοιπές» δίνουν τη διαφορά', Math.abs(sum - c.diff) < 0.001);
  ok('υπάρχει υπόλοιπο όταν κόβονται κατηγορίες', Math.abs(c.rest) > 0);
}
