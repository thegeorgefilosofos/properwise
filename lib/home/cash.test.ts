...
// Είναι η θέση όπου αλλού θα έμπαινε χρώμα. Πρέπει να λέει κάτι μετρήσιμο.
{
  ...
  ok('«λήγει σήμερα», όχι «σε 0 ημέρες»', cashSideNote(today.owedByMe, 'out') === '1 απλήρωτο · η πρώτη λήγει σήμερα')
  const soon = cashPosition({ ...empty, payables: [pay({ due: '2026-08-06' })] })
  ok('ενικός και στην επερχόμενη', cashSideNote(soon.owedByMe, 'out') === '1 απλήρωτο · η πρώτη σε 1 ημέρα')
  const noDue = cashPosition({ ...empty, payables: [pay({ due: null })] })
  ok('χωρίς προθεσμία, χωρίς εφευρημένη ημερομηνία', cashSideNote(noDue.owedByMe, 'out') === '1 απλήρωτο')
  ...
  // επείγουσα διάκριση την κουβαλούσε η ατζέντα, χωρίς ποσό.
  {
    ...
    ok('λέει το ληξιπρόθεσμο ποσό όταν είναι μέρος',
      cashSideNote(mixed.owedByMe, 'out') === `2 απλήρωτα · ${fe(120)} ληξιπρόθεσμα, η παλαιότερη 18 ημέρες πίσω`)
  }
  {
    ...
    ok('δεν επαναλαμβάνει το σύνολο όταν όλα είναι ληξιπρόθεσμα',
      cashSideNote(all.owedByMe, 'out') === '2 απλήρωτα · η παλαιότερη 18 ημέρες πίσω')
  }
}
