...
// ── ΤΟ ΚΕΝΟ ΑΚΙΝΗΤΟ ΔΕΝ ΕΙΝΑΙ ΣΦΑΛΜΑ ──────────────────────────────────────
{
  ...
  ok('χωρίς εκκρεμείς πληρωμές', cashSideNote(r.owedByMe, 'out') === 'Τίποτα απλήρωτο')
}
...
// Είναι η θέση όπου αλλού θα έμπαινε χρώμα. Πρέπει να λέει κάτι μετρήσιμο.
{
  ...
  ok('ενικός στη μία δόση', cashSideNote(one.owedToMe, 'in') === '1 δόση · η παλαιότερη 1 ημέρα πίσω')
  const two = cashPosition({ ...empty, rent: [rent({ due_date: '2026-07-05', period_month: 7 }), rent({ due_date: '2026-08-01', period_month: 8 })] })
  ok('πληθυντικός και σωστό «παλαιότερη»', cashSideNote(two.owedToMe, 'in') === '2 δόσεις · η παλαιότερη 31 ημέρες πίσω')
  const today = cashPosition({ ...empty, payables: [pay({ due: TODAY })] })
  ok('«λήγει σήμερα», όχι «σε 0 ημέρες»', cashSideNote(today.owedByMe, 'out') === '1 απλήρωτος · ο πρώτος λήγει σήμερα')
  const soon = cashPosition({ ...empty, payables: [pay({ due: '2026-08-06' })] })
  ok('ενικός και στην επερχόμενη', cashSideNote(soon.owedByMe, 'out') === '1 απλήρωτος · ο πρώτος σε 1 ημέρα')
  const noDue = cashPosition({ ...empty, payables: [pay({ due: null })] })
  ok('χωρίς προθεσμία, χωρίς εφευρημένη ημερομηνία', cashSideNote(noDue.owedByMe, 'out') === '1 απλήρωτος')
  ...
  // επείγουσα διάκριση την κουβαλούσε η ατζέντα, χωρίς ποσό.
  {
    ...
    ok('λέει το ληξιπρόθεσμο ποσό όταν είναι μέρος',
      cashSideNote(mixed.owedByMe, 'out') === `2 απλήρωτοι · ${fe(120)} ληξιπρόθεσμα, ο παλαιότερος 18 ημέρες πίσω`)
  }
  {
    ...
    ok('δεν επαναλαμβάνει το σύνολο όταν όλα είναι ληξιπρόθεσμα',
      cashSideNote(all.owedByMe, 'out') === '2 απλήρωτοι · ο παλαιότερος 18 ημέρες πίσω')
  }
}
