...
export function cashSideNote(s: CashSide, kind: 'in' | 'out'): string {
  ...
  // πράγματα.
  //
  // ΚΑΙ ΤΟ ΓΕΝΟΣ ΑΚΟΛΟΥΘΕΙ ΤΟ ΟΥΣΙΑΣΤΙΚΟ. Η ουρά έγραφε «η παλαιότερη» και για
  // τις δύο πλευρές: σωστό για τις δόσεις, λάθος δίπλα στα «απλήρωτα». Η πλευρά
  // «Χρωστάω» μετρά λογαριασμούς, με την ίδια λέξη που γράφουν οι Δαπάνες.
  const items = kind === 'in'
    ? `${s.count} ${s.count === 1 ? 'δόση' : 'δόσεις'}`
    : `${s.count} ${s.count === 1 ? 'απλήρωτος' : 'απλήρωτοι'}`;
  const oldest = kind === 'in' ? 'η παλαιότερη' : 'ο παλαιότερος';
  const firstOne = kind === 'in' ? 'η πρώτη' : 'ο πρώτος';
  if (d != null && d < 0) {
    ...
    const tail = `${oldest} ${late} ${late === 1 ? 'ημέρα' : 'ημέρες'} πίσω`;
    return s.overdueCount > 0 && s.overdueCount < s.count
      ? `${items} · ${fe(s.overdue)} ληξιπρόθεσμα, ${tail}`
      : `${items} · ${tail}`;
  }
  if (d != null) {
    if (d === 0) return `${items} · ${firstOne} λήγει σήμερα`;
    return `${items} · ${firstOne} σε ${d} ${d === 1 ? 'ημέρα' : 'ημέρες'}`;
  }
  return items;
}
