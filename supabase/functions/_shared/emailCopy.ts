...
export const ONBOARDING: Record<string, CopyFn> = {
  ...
  welcome_free: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Καλωσόρισες') + h('Ας ξεκινήσουμε') + greeting(c.name)
        + p('Από σήμερα, εσύ καταχωρείς τα βασικά και το PROPERWISE κάνει τα υπόλοιπα: έσοδα, έξοδα, φόρους και υπενθυμίσεις.')
        ...
        + bullets([
            ...
            'Ρώτα τη Νόα ό,τι σε απασχολεί, όπως θα ρωτούσες έναν άνθρωπο.',
          ])
  ...
  // 2. Καλωσόρισμα, προφίλ ιδιώτη (ημέρα 0)
  welcome_individual: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Καλωσόρισες') + h('Ας τα τακτοποιήσουμε') + greeting(c.name)
        + p('Από σήμερα, τα ενοίκια, τα έξοδα και οι υποχρεώσεις σου καταγράφονται ανά ακίνητο. Δεν χρειάζεται πια να ψάχνεις σε σημειώσεις, μηνύματα και αποδείξεις: εσύ κρατάς τον έλεγχο, το PROPERWISE κάνει τη δουλειά ρουτίνας.')
  ...
  // 3. Καλωσόρισμα, προφίλ επαγγελματία (ημέρα 0)
  welcome_professional: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Καλωσόρισες') + h('Ώρα να δουλέψουμε') + greeting(c.name)
        + p('Το PROPERWISE φτιάχτηκε για επαγγελματίες που διαχειρίζονται πολλά ακίνητα και δεν έχουν χρόνο για χαμένες κινήσεις. Από τις εισπράξεις μέχρι τη λογιστική, τα βλέπεις ανά ακίνητο και συνολικά.')
        + (has(c.properties)
            ? p(`Έχεις ήδη <b>${plural(c.properties, 'ακίνητο', 'ακίνητα')}</b> στο χαρτοφυλάκιό σου. Ας τα κάνουμε να δουλεύουν για σένα.`)
            : '')
        + p('Τι έχει το PROPERWISE για επαγγελματίες:')
        + bullets([
  ...
  // 9. Συμβουλή: Νόα για τα ακίνητα (ημέρα 5)
  tip_assistant: (c) => {
    return {
      subject: 'Νόα ξέρει τα ακίνητά σου',
      html: emailShell({
        preheader: 'Από φόρους μέχρι αποδόσεις, με τα δικά σου δεδομένα.',
        unsubUrl: c.unsubUrl,
        bodyHtml: eyebrow('Νόα') + h('Έχεις μια απορία;') + greeting(c.name)
          + p('Νόα ξέρει τα ακίνητά σου. Ρώτα πόσο φόρο θα πληρώσεις, αν συμφέρει ένα δάνειο ή τι απόδοση βγάζει ένα ακίνητο και παίρνεις απάντηση με βάση τα δικά σου δεδομένα.')
          + p('Γράψε ή μίλα ελεύθερα, στα ελληνικά. Πες τι θες, όπως θα το έλεγες σε έναν άνθρωπο.')
...
export const UPSELL: Record<string, CopyFn> = {
  ...
  // 25. Αναβάθμιση ιδιώτη χωρίς συνδρομή. Η SQL το στέλνει σε όποιον έχει 2+
  // ακίνητα, οπότε το πακέτο που του ταιριάζει είναι το «Ιδιοκτήτης+»: το
  // «Ιδιώτης» είναι τρόπος χρήσης και δεν αγοράζεται.
  upsell_to_individual: (c) => {
    const pack = `«${PACKAGE_NAME.owner}»`;
    const proof = has(c.properties)
      ? p(`Έχεις ήδη <b>${plural(c.properties, 'ακίνητο', 'ακίνητα')}</b> στο PROPERWISE. Με το ${pack}, το καθένα δουλεύει στο έπακρο για σένα.`)
      : p(`Χρησιμοποιείς ήδη τα βασικά. Με το ${pack} έχεις όσα κάνουν τη διαχείριση πραγματικά αβίαστη.`);
    return { subject: 'Δώσε στα ακίνητά σου το πλήρες PROPERWISE', html: emailShell({
      ...
      unsubUrl: c.unsubUrl,
      bodyHtml: eyebrow(`Πακέτο ${PACKAGE_NAME.owner}`) + h('Ώρα για το επόμενο βήμα') + greeting(c.name)
        + proof
        ...
        + button(`Δες το ${pack}`, dash(c))
        + note(NOTE.cancel),
    }) };
  },

  // 26. Αναβάθμιση προς το πακέτο «Επαγγελματίας»
  ...
  // 27. Έφτασε το όριο ακινήτων του δωρεάν πακέτου
  limit_reached: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Χώρος για ανάπτυξη') + h('Χρειάζεσαι λίγο περισσότερο χώρο') + greeting(c.name)
        // Το δωρεάν πακέτο και το «Ιδιοκτήτης με Νόα» κρατούν από ένα ακίνητο:
        // το επόμενο σκαλί που δίνει χώρο είναι το «Ιδιοκτήτης+», όχι το φθηνότερο.
        + p(`Το δωρεάν πακέτο «Ιδιοκτήτης» κρατά ένα ακίνητο. Για δεύτερο και τρίτο υπάρχει το «${PACKAGE_NAME.owner}», με έως ${PACKAGE_MAX_PROPERTIES.owner} ακίνητα και τη Νόα μέσα.`)
        + p('Κρατάς ό,τι έχεις καταχωρήσει. Φεύγει μόνο το όριο.')
        + button('Δες τα πακέτα', dash(c))
        + note(NOTE.cancel),
    }),
  }),
  ...
  trial_started: (c) => {
    ...
    // Η δοκιμή τρέχει ΠΑΝΤΑ στο TRIAL_PLAN (lib/billing/entitlements.ts), όποιο
    ...
    return { subject: 'Η δοκιμή σου ξεκίνησε. Ας την αξιοποιήσουμε', html: emailShell({
      ...
      bodyHtml: eyebrow('Δοκιμή') + h('Καλή αρχή. Πάμε να δεις την αξία') + greeting(c.name)
        ...
        + bullets([
            ...
            'Ρώτα τη Νόα κάτι για τα ακίνητά σου.',
          ])
...
export const LIFECYCLE: Record<string, CopyFn> = {
  ...
  reply_ack: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Επικοινωνία') + h('Το μήνυμά σου έφτασε') + greeting(c.name)
        ...
        + p('Αν είναι κάτι επείγον για κάποιο ακίνητό σου, μπορείς εν τω μεταξύ να ρωτήσεις τη Νόα μέσα στην εφαρμογή. Ξέρει τα δεδομένα σου και απαντά αμέσως.')
        + button('Ρώτα τη Νόα', dash(c))
  ...
  // κυκλοφορήσει· το κείμενο μιλά σε μέλλοντα χρόνο, όπως η αρχική και Νόα.
  ...
  mobile_launch: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Έρχεται') + h('Τα ακίνητά σου, παντού μαζί σου') + greeting(c.name)
        + p('Ετοιμάζουμε την εφαρμογή για κινητά. Οι εισπράξεις, οι ειδοποιήσεις και τα ακίνητά σου, στην τσέπη σου, όπου κι αν βρίσκεσαι.')
        + bullets([
            'Ειδοποιήσεις σε πραγματικό χρόνο για εισπράξεις και προθεσμίες.',
            'Γρήγορη καταχώρηση εσόδων και εξόδων με λίγα αγγίγματα.',
            'Ρώτα τη Νόα για τα ακίνητά σου, από όπου κι αν είσαι.',
          ])
