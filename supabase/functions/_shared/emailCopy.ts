  ...
  annual_discount: (c) => {
    ...
    return { subject: `Πλήρωσε ετησίως και εξοικονόμησε ${pct}%`, html: emailShell({
      ...
      bodyHtml: eyebrow('Ετήσια χρέωση') + h('Λιγότερο κόστος με την ετήσια συνδρομή') + greeting(c.name)
        + p(`Αν το PROPERWISE έγινε μέρος της καθημερινότητάς σου, η ετήσια χρέωση σου χαρίζει <b>${pct}%</b> έκπτωση σε σχέση με τη μηνιαία. Ίδιες δυνατότητες, μικρότερο κόστος.`)
...
export const LIFECYCLE: Record<string, CopyFn> = {
  ...
  accountant_request_answered: (c) => ({
    subject: `${c.clientName || 'Ο πελάτης σου'}: ήρθε αυτό που ζήτησες`,
    html: emailShell({
  ...
  mobile_launch: (c) => ({
    subject: 'Το PROPERWISE είναι πλέον στο κινητό σου',
    html: emailShell({
  ...
  inspection_due: (c) => {
    ...
    return { subject: 'Ώρα για τον ετήσιο έλεγχο του ακινήτου', html: emailShell({
      ...
      bodyHtml: eyebrow('Έλεγχος') + h('Ένας έλεγχος αξίζει τον κόπο') + greeting(c.name)
        + p(`Πέρασε ένας χρόνος. Ένας σύντομος έλεγχος${ref}, για φθορές, ασφάλεια και μικροεπισκευές, προλαμβάνει μεγαλύτερα έξοδα και κρατά την αξία του ψηλά.`)
...
export const SHORTTERM: Record<string, CopyFn> = {
  ...
  str_season_recap: (c) => {
    const bits: string[] = [];
    if (has(c.collected)) bits.push(`έσοδα <b>${eur(c.collected)}</b>`);
    if (has(c.occupancy)) bits.push(`πληρότητα <b>${c.occupancy}%</b>`);
    ...
    return {
      ...
      html: emailShell({
        ...
        bodyHtml: eyebrow('Συνδρομή')
          + h(first ? 'Ξεκινά η συνδρομή σου' : 'Ερχεται η ανανέωση')
          + greeting(c.name)
          + p(first
    ...
    return {
      ...
      html: emailShell({
        ...
        bodyHtml: eyebrow('Λογαριασμός')
          ...
          + button('Ανοιξε τον λογαριασμό σου', dash(c))
...
export const VALUE: Record<string, CopyFn> = {
  ...
  yield_boost: (c) => {
    ...
    const lead = has(c.occupancy)
      ? p(`Η πληρότητα${ref} κινείται γύρω στο <b>${c.occupancy}%</b>. Με λίγες στοχευμένες κινήσεις, η απόδοση ανεβαίνει χωρίς μεγάλη επένδυση:`)
