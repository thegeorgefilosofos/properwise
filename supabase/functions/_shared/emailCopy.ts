...
export const ONBOARDING: Record<string, CopyFn> = {
  ...
  // 1. Καλωσόρισμα, δωρεάν δοκιμή (ημέρα 0)
  welcome_free: (c) => ({
  ...
  // 2. Καλωσόρισμα, πακέτο Ιδιώτη (ημέρα 0)
  ...
  // 3. Καλωσόρισμα, πακέτο Επαγγελματία (ημέρα 0)
...
export const UPSELL: Record<string, CopyFn> = {
  ...
  // 24β. Μηνιαία παρότρυνση αναβάθμισης (χρήστες χωρίς συνδρομή)
  free_month_upgrade: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Αναβάθμιση') + h('Δώσε στο ακίνητό σου περισσότερα') + greeting(c.name)
        + p('Άλλος ένας μήνας χωρίς συνδρομή. Μήπως ήρθε η ώρα να ξεκλειδώσεις περισσότερες δυνατότητες για το ακίνητό σου, να έχεις περισσότερα δεδομένα και να παίρνεις σωστότερες αποφάσεις;')
        + p('Με το πακέτο Ιδιώτη ή Επαγγελματία χρησιμοποιείς εργαλεία που σου εξοικονομούν χρόνο και χρήματα, κάνοντας το ακίνητό σου πιο αποδοτικό, με λίγα μόλις κλικ από το κινητό σου.')
  ...
  upsell_to_individual: (c) => {
    const proof = has(c.properties)
      ? p(`Έχεις ήδη <b>${plural(c.properties, 'ακίνητο', 'ακίνητα')}</b> στο PROPERWISE. Με το πακέτο Ιδιώτη, το καθένα δουλεύει στο έπακρο για σένα.`)
      : p('Χρησιμοποιείς ήδη τα βασικά. Με το πακέτο Ιδιώτη ξεκλειδώνεις όσα κάνουν τη διαχείριση πραγματικά αβίαστη.');
    return { subject: 'Δώσε στα ακίνητά σου το πλήρες PROPERWISE', html: emailShell({
      ...
      unsubUrl: c.unsubUrl,
      bodyHtml: eyebrow('Πακέτο Ιδιώτη') + h('Ώρα για το επόμενο βήμα') + greeting(c.name)
        + proof
  ...
  upsell_to_professional: (c) => {
    const proof = (has(c.properties) && c.properties >= 2)
      ? p(`Διαχειρίζεσαι <b>${plural(c.properties, 'ακίνητο', 'ακίνητα')}</b>. Όσο μεγαλώνει το χαρτοφυλάκιό σου, τόσο περισσότερο κερδίζεις από το πακέτο Επαγγελματία.`)
      : p('Καθώς το χαρτοφυλάκιό σου μεγαλώνει, το πακέτο Επαγγελματία σε κρατά μπροστά χωρίς επιπλέον κόπο.');
  ...
  // 27. Έφτασε το όριο ακινήτων χωρίς συνδρομή
  limit_reached: (c) => ({
    subject: 'Έφτασες στο όριο των ακινήτων σου',
    html: emailShell({
      ...
      bodyHtml: eyebrow('Χώρος για ανάπτυξη') + h('Χρειάζεσαι λίγο περισσότερο χώρο') + greeting(c.name)
        + p('Έφτασες ως εδώ χωρίς συνδρομή· αυτό είναι καλό σημάδι: το χρησιμοποιείς σοβαρά. Για να συνεχίσεις χωρίς περιορισμούς, μια αναβάθμιση ανοίγει τον δρόμο.')
  ...
  value_left: (c) => {
    ...
    return { subject: 'Αφήνεις αξία αναξιοποίητη', html: emailShell({
      preheader: 'Δες τι μένει ανεκμετάλλευτο στο πακέτο σου.',
      unsubUrl: c.unsubUrl,
  ...
  winback_downgrade: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Καλωσόρισες πίσω') + h('Η πόρτα μένει ανοιχτή') + greeting(c.name)
        + p('Πέρασες σε πιο βασικό πακέτο και το σεβόμαστε απόλυτα. Τα ακίνητα, οι εισπράξεις και το ιστορικό σου παραμένουν ακέραια και δικά σου.')
