...
export const PACKAGE_NAME = {
  solo: 'Ιδιοκτήτης με Νόα', owner: 'Ιδιοκτήτης+', agency: 'Επαγγελματίας', office: 'Επαγγελματίας+',
} as const;
...
function packagePerks(pkg: PackageId): string[] {
  ...
  switch (pkg) {
    // Από 25.09.2026 ο «Ιδιοκτήτης» χωρίς Νόα είναι δωρεάν· αυτό που προσθέτει
    // το πακέτο είναι ο βοηθός και η σάρωση χωρίς όριο.
    case 'solo': return [`${limit}.`, 'Ψηφιακός βοηθός Νόα, για κάθε ερώτηση για το ακίνητό σου.', 'Σάρωση εγγράφων χωρίς μηνιαίο όριο.'];
...
export function planDowngradedEmail(c: Ctx & { plan: Plan | PackageId }): Out {
  const pkg = packageOf(c.plan);
  // ΧΩΡΙΣ ΣΥΝΔΡΟΜΗ ΔΕΝ ΣΗΜΑΙΝΕΙ «ΣΤΑΜΑΤΗΣΕ». Από 25.09.2026 ο λογαριασμός
  // συνεχίζει στο δωρεάν πακέτο «Ιδιοκτήτης» (ένα ακίνητο, τα φορολογικά,
  // σαρώσεις με μηνιαίο όριο, χωρίς Νόα) και δεν διαγράφεται.
  const title = pkg ? `Το πακέτο σου είναι τώρα ${PACKAGE_NAME[pkg]}` : 'Συνεχίζεις στο δωρεάν πακέτο Ιδιοκτήτης';
  const body = eyebrow('Αλλαγή πακέτου') + h(title)
    + greeting(c.name)
    + p('Καταγράψαμε την αλλαγή. Τα δεδομένα σου μένουν στον λογαριασμό σου.')
    + (pkg ? '' : p('Κρατάς ένα ακίνητο με όλα τα φορολογικά του: Ε2 για τον λογιστή, ΕΝΦΙΑ, ενοικιαστή και εισπράξεις. Ο ψηφιακός βοηθός Νόα δεν περιλαμβάνεται στο δωρεάν πακέτο.'))
