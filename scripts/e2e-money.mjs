...
// ── 5 ────────────────────────────────────────────────────────────────────
// Ο ΤΡΟΠΟΣ ΕΙΣΠΡΑΞΗΣ ΘΑ ΑΛΛΑΞΕΙ ΤΟΝ ΦΟΡΟ ΑΠΟ ΤΟ 2027 (ν.5222/2025, άρθρο 210,
// κύρωση από 1.7.2027 με την Α.1187/2026). Δεν επιτρέπεται ούτε να προεπιλεγεί
// σιωπηλά η κερδοφόρα εκδοχή, ούτε να γραφτεί άλλο από αυτό που είδε ο ιδιοκτήτης,
// ούτε να αναφερθεί λάθος νόμος (ο ν.5246/2025 είναι μόνο η κλίμακα).
{
  const s = await open('rent-cash');
  const body = await bodyText(s.page);
  ok('5. με μετρητά, η οθόνη ενημερώνει για την έκπτωση 5% με τον σωστό νόμο',
    body.includes('τεκμαρτή έκπτωση 5%') && body.includes('5222') && !body.includes('5246'));
  await s.primary().click();
  await settle(s.page);
  const w = await s.writes('rent_payments');
