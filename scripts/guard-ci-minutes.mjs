 ...
 *
 *     Σαρώσεις περιηγητή             10΄10΄΄ → 11
 *     verify                         13΄46΄΄ → 14
 *     Ο,τι κρίνεται αφού ζωγραφιστεί 21΄14΄΄ → 22
 ...
 *                                            ───
 *                                             83 λεπτά ανά εκτέλεση
 *
 * ΤΙ ΑΓΟΡΑΣΑΝ ΤΑ ΤΕΣΣΕΡΑ ΛΕΠΤΑ ΠΟΥ ΠΡΟΣΤΕΘΗΚΑΝ. Εξι σαρωτές κάθονταν έξω από
 * το CI με δικαιολογίες που είχαν πάψει να ισχύουν, ανάμεσά τους οι διαδρομές
 * που κινούν ευρώ. Μετρημένος χρόνος: 21΄΄ στις σαρώσεις (χωρίς διακομιστή,
 * το job έχει ήδη Chromium) και 138΄΄ στο «ό,τι κρίνεται αφού ζωγραφιστεί»
 * (έχει ήδη χτίσιμο και ζωντανό διακομιστή). Μπήκαν σε ΥΠΑΡΧΟΝΤΑ jobs και όχι
 * σε καινούρια: ένα νέο job θα πλήρωνε δεύτερη φορά εγκατάσταση και χτίσιμο,
 * που κοστίζουν πολλαπλάσια από τις ίδιες τις σαρώσεις.
 *
 * Το τίμημα σε εκτελέσεις: από 22 τον μήνα σε 21. Το κέρδος: 235 έλεγχοι που
 * μέχρι σήμερα δεν έτρεχε κανείς — και η προηγούμενη φορά που ένας σαρωτής
 * έμεινε έξω από το CI κόστισε δεκαοκτώ commits με υποχώρηση μέσα τους.
 *
 * ΜΕΤΡΗΘΗΚΑΝ ΣΕ ΑΛΗΘΙΝΗ ΕΚΤΕΛΕΣΗ (08/09/2026, run 34221350116) και τα τρία νέα
 * βήματα βγήκαν όσο τα είχαμε υπολογίσει: χρήματα 20΄΄, δημόσια/προσβασιμότητα/
 * αρχική/βάρος 96΄΄, κινητό και ταμπλέτα 44΄΄. Οι σαρώσεις πήγαν 11΄21΄΄ σε όριο
 * 20΄ και το «ό,τι κρίνεται αφού ζωγραφιστεί» 15΄59΄΄ σε όριο 30΄.
 *
 * ΤΑ ΝΟΥΜΕΡΑ ΑΝΑ ΔΟΥΛΕΙΑ ΠΑΡΑΠΑΝΩ ΕΙΝΑΙ ΑΠΑΙΣΙΟΔΟΞΑ ΚΑΙ ΜΕΝΟΥΝ. Η ίδια αληθινή
 * εκτέλεση έδωσε verify 6΄ (εδώ 14) και ζωγραφιστεί 16΄ (εδώ 22): οι δρομείς του
 * GitHub διαφέρουν σε ταχύτητα και σε θερμότητα της μνήμης npm. Ενας φύλακας
 * προϋπολογισμού που κρατά το ΧΕΙΡΟΤΕΡΟ παρατηρημένο κοστίζει μια εκτέλεση τον
 * μήνα σε ακρίβεια· ένας που κρατά το καλύτερο κοστίζει ολόκληρο το CI.
 *
