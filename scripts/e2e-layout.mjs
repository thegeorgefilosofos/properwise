...
const PROBE = () => {
  ...
  for (const g of document.querySelectorAll('*')) {
    ...
    // ═══ ΚΑΙ Η ΡΑΓΙΣΜΕΝΗ ΣΕΙΡΑ ΧΕΙΡΙΣΤΗΡΙΩΝ, ΠΟΥ ΤΟ `sameW` ΤΗΝ ΕΚΡΥΒΕ ═══════
    // Ο ΕΛΕΓΧΟΣ ΑΠΟ ΚΑΤΩ ΖΗΤΑΕΙ ΙΔΙΟ ΠΛΑΤΟΣ. Τα τσιπάκια και τα κουμπιά
    // παίρνουν πλάτος από το κείμενό τους — «Εκκρεμεί» δεν είναι «Πληρώθηκε» —
    // οπότε ΚΑΜΙΑ σειρά χειριστηρίων δεν έφτανε ποτέ ώς εδώ. Ο χρήστης
    // φωτογράφισε έξι από αυτές σε ταμπλέτα: 3+2, 4+1, 5+2, 6+1.
    //
    // Η ΟΙΚΟΓΕΝΕΙΑ ΔΕΝ ΛΕΓΕΤΑΙ ΜΕ ΤΟ ΠΛΑΤΟΣ, ΛΕΓΕΤΑΙ ΜΕ ΤΟ ΥΨΟΣ. Πέντε κουμπιά
    // του ίδιου ύψους, όλα πατήσιμα, μέσα στο ίδιο δοχείο, είναι ΕΝΑ σύνολο και
    // διαβάζονται ως σύνολο. Οταν τυλίγονται άνισα, η τελευταία σειρά κρέμεται
    // αριστερά με τρύπα δεξιά — η ίδια παραφωνία με τα πλακίδια, σε άλλο ρούχο.
    if (rows.size >= 2 && !sameW && kids.length <= 12) {
      const ctrl = kids.every(k => k.tagName === 'BUTTON' || k.tagName === 'A' || k.getAttribute('role') === 'button')
      const h0 = Math.round(boxes[0].height)
      const sameH = boxes.every(x => Math.abs(x.height - h0) <= 2)
      // ΡΑΓΙΣΜΕΝΗ ΕΙΝΑΙ Η ΣΕΙΡΑ ΠΟΥ ΔΕΝ ΦΤΑΝΕΙ ΤΗ ΔΕΞΙΑ ΑΚΡΗ, όχι απλώς αυτή που
      // έχει λιγότερα στοιχεία. Οταν το τελευταίο κουμπί απλώνεται σε όλο το
      // πλάτος, η γραμμή είναι γεμάτη και το μάτι δεν βλέπει τρύπα — αλλιώς ο
      // έλεγχος θα κατήγγελλε ως ελάττωμα ακριβώς τη σωστή λύση.
      const gr = g.getBoundingClientRect()
      const lines = new Map()
      let ln = 0, lb = -Infinity
      for (const b of boxes) {
        if (b.top >= lb - 1) { ln++; lb = b.bottom } else lb = Math.max(lb, b.bottom)
        lines.set(ln, Math.max(lines.get(ln) ?? -Infinity, b.right))
      }
      // ΚΑΙ Η ΠΡΩΤΗ ΓΡΑΜΜΗ ΠΡΕΠΕΙ ΝΑ ΦΤΑΝΕΙ ΤΗΝ ΑΚΡΗ, ΑΛΛΙΩΣ ΔΕΝ ΕΙΝΑΙ ΣΕΙΡΑ.
      // ΨΕΥΔΕΣ ΕΥΡΗΜΑ, ΔΙΚΟ ΜΟΥ: ο έλεγχος κατήγγειλε 390 φορές τα φίλτρα των
      // Εγγράφων («Νερό 5», «Ρεύμα 5», «Bosch 2»…). Αυτά είναι ομάδα τσιπακίων
      // στοιχισμένη ΑΡΙΣΤΕΡΑ, δίπλα στην ετικέτα της: δεν φτάνει την άκρη ΚΑΜΙΑ
      // γραμμή τους, ούτε η πρώτη, γιατί δεν είναι φτιαγμένα να τη φτάνουν. Θα
      // ήταν λάθος να απλωθεί το «Νερό 5» στο ένα τέταρτο της οθόνης.
      //
      // Σειρά που ΡΑΓΙΖΕΙ είναι αυτή που γεμίζει την πρώτη της γραμμή — άρα
      // δηλώνει ότι θέλει όλο το πλάτος — και μετά αφήνει την επόμενη μισή.
      const rowRights = [...lines.values()]
      const firstFull = gr.right - rowRights[0] <= 24
      const short = rowRights.slice(1).some(r => gr.right - r > 24)
      // ΚΑΙ ΤΟ ΣΥΝΝΕΦΟ ΦΙΛΤΡΩΝ ΔΕΝ ΕΙΝΑΙ ΣΕΙΡΑ ΧΕΙΡΙΣΤΗΡΙΩΝ. Δεύτερο ψευδές
      // εύρημα, δικό μου: τα φίλτρα των Εγγράφων («Νερό 5», «Ρεύμα 5», «ΔΕΗ 1»)
      // γεμίζουν την πρώτη γραμμή και αφήνουν τη δεύτερη μισή. Αυτό ΕΙΝΑΙ το
      // σωστό: ένα σύννεφο ετικετών τυλίγεται, δεν απλώνεται — το «Νερό 5» στο
      // ένα τέταρτο της οθόνης θα ήταν παραμόρφωση.
      //
      // ΔΟΚΙΜΑΣΤΗΚΕ ΝΑ ΞΕΧΩΡΙΣΟΥΝ ΜΕ ΤΟ ΠΛΑΤΟΣ ΚΑΙ ΔΕΝ ΑΝΤΕΧΕΙ. Μετρημένο στα
      // 440: τα φίλτρα βγάζουν μέσο πλάτος 102 σε δοχείο 416 (24%) και οι φακοί
      // 242 (58%) — φαίνεται καθαρό, αλλά μια σειρά πέντε χειριστηρίων που
      // τυλίγεται 4+1 δίνει 25%, δηλαδή ακριβώς πάνω στο ίδιο νούμερο. Κατώφλι
      // που ξεχωρίζει το σωστό από το λάθος κατά μία μονάδα δεν είναι κατώφλι.
      //
      // Η ΔΗΛΩΣΗ ΕΙΝΑΙ ΤΟ ΚΡΙΤΗΡΙΟ. Οι σειρές που ΟΦΕΙΛΟΥΝ να γεμίζουν το πλάτος
      // τους φοράνε κλάση που το λέει: `lens-bar`, `po-ctlrow`, `act-row`. Ο
      // έλεγχος φυλάει το ιδίωμα — ό,τι δεν το δηλώνει, δεν κρίνεται από αυτό.
      const declared = /\b(lens-bar|po-ctlrow|act-row)\b/.test(g.className || '')
      const counts2 = [...rows.values()]
      if (ctrl && sameH && declared && firstFull && short && new Set(counts2).size > 1)
        add('ΡΑΓΙΣΜΕΝΗ ΣΕΙΡΑ', kids.map(k => (k.textContent || '').trim().slice(0, 12)).join('/').slice(0, 60), counts2.join('+'))
    }
...
async function scanDevice(dev, out) {
  ...
  for (const s of (ONLY ? SCENES.filter(x => ONLY.includes(x)) : SCENES)) {
    ...
    //
    // ΚΑΙ ΡΩΤΑΕΙ ΔΕΥΤΕΡΗ ΦΟΡΑ ΠΡΙΝ ΚΑΤΗΓΟΡΗΣΕΙ. Μετρημένο στις 07/09/2026: η
    // σκηνή `compare` αναφέρθηκε ΚΕΝΗ (0 χαρακτήρες) στα 393×852, ενώ πέντε
    // χειροκίνητοι γύροι στο ίδιο παράθυρο έδωσαν 716 χαρακτήρες κάθε φορά και
    // η ίδια η σάρωση με `E2E_ONLY=compare` βγήκε καθαρή σε ΟΛΕΣ τις 22
    // συσκευές. Η διαφορά ήταν ο φόρτος: έτρεχαν δύο ροές πρακτόρων κι ένα
    // χτίσιμο στο ίδιο μηχάνημα, οπότε η απόδοση άργησε πιο πολύ από την
    // αναμονή. Ενα ψευδώς θετικό σε πύλη κοστίζει όσο κι ένα ψευδώς αρνητικό:
    // στέλνει τον άνθρωπο να ψάξει ελάττωμα που δεν υπάρχει.
    //
    // Η δεύτερη ερώτηση ΔΕΝ αδυνατίζει τον έλεγχο: μια σκηνή που όντως σκάει
    // στην απόδοση μένει κενή όσο κι αν την ξαναρωτήσεις. Οποια γεμίζει στα
    // 1.500 επιπλέον χιλιοστά του δευτερολέπτου δεν ήταν κενή, ήταν αργή.
    const measure = () => p.evaluate(() => (document.querySelector('.app-content')?.innerText || '').trim().length)
    let size = await measure()
    if (size < 120) { await p.waitForTimeout(1500); size = await measure() }
