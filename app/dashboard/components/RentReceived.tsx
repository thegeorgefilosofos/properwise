...
//
// Ο ΤΡΟΠΟΣ ΕΙΣΠΡΑΞΗΣ ΡΩΤΙΕΤΑΙ, ΓΙΑΤΙ ΘΑ ΑΛΛΑΞΕΙ ΤΟΝ ΦΟΡΟ. Από 1.7.2027
// (ν.5222/2025) η τεκμαρτή έκπτωση 5% θα προϋποθέτει είσπραξη με τραπεζικό ή
...
export default function RentReceived({
  ...
  return (
    <Modal open onClose={() => { if (!busy) onClose(); }} size="sm"
      ...
      {/* ΤΡΕΙΣ ΓΡΑΜΜΕΣ ΜΕ ΡΙΓΜΕΝΗ ΔΕΞΙΑ ΑΚΡΗ. Το παράθυρο είναι `sm`, δηλαδή 440
          εικονοστοιχεία· μετά το γέμισμα του παραθύρου, το γέμισμα του πλαισίου
          και την κουκκίδα, στο κείμενο μένουν 344. Οι 153 χαρακτήρες πιάνουν
          τρεις γραμμές εκεί μέσα. Το `po-just` κλείνει τη δεξιά άκρη — μπαίνει
          στο κείμενο μέσω του `className` του InfoBanner, όχι στο πλαίσιο — και
          το `hy()` δίνει τα σημεία τομής ώστε η στοίχιση να κλείνει τα κενά
          αντί να τα τεντώνει. */}
      {method === 'Μετρητά' && (
