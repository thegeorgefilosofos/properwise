...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  return new Response(ics, {
    ...
    headers: {
      ...
      // Μία ώρα είναι το ΤΑΒΑΝΙ μιας αποθηκευμένης απάντησης, όχι ο ρυθμός
      // ανανέωσης. Πόσο συχνά ξαναρωτά το ημερολόγιο το ορίζει το
      // REFRESH-INTERVAL / X-PUBLISHED-TTL του ίδιου του αρχείου
      // (DEFAULT_TTL_HOURS, lib/calendar/feed.ts): μια νέα προθεσμία φαίνεται
      // σε ως εκείνες τις ώρες και η Google, που αγνοεί την υπόδειξη, ανανεώνει
      // με δικό της ρυθμό. Για προθεσμίες που απέχουν μέρες ή εβδομάδες αρκεί·
      // το σχόλιο έλεγε «την ίδια μέρα», που δεν το εγγυάται κανείς.
