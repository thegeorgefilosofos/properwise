...
| **GitHub** (GitHub, Inc. / Microsoft) | Source control, CI/CD, encrypted DB-backup artifacts | Source code; backup artifacts contain customer data (encrypted when `BACKUP_PASSPHRASE` set) | US (SCCs required) | ☐ No DPA on the Free plan. Backups are GPG-encrypted (AES256, `backup-watchdog.yml` green on 22 and 23.09.2026), **but symmetrically, with `BACKUP_PASSPHRASE` stored as a GitHub Actions secret** (`db-backup.yml`): GitHub holds both ciphertext and key, so the encryption is not a supplementary measure against GitHub itself. Open: public-key encryption with the private key kept offline, or a signed DPA |
