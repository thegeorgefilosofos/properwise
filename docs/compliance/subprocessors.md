...
| **Vercel** (Vercel Inc.) | Application hosting and delivery — every request transits it en route to the database | Request metadata, IP; application traffic in transit | US — global edge network | Incorporated in the ToS, but covers Pro/Enterprise; Hobby is non-commercial use only. ☐ confirm the project runs on Pro, then archive the DPA PDF |
| **Google** (Google LLC) | Google OAuth sign-in (`app/login/page.tsx`, `app/signup/page.tsx`). The contact dossier map is now a plain link the user clicks (`TabContacts.tsx`), no iframe; Google Fonts were removed 2026-07-27 | OAuth: email, name, IP | US | Google acts as independent controller under the Google APIs Terms; no processor DPA exists for Sign in with Google. ✅ nothing to sign |
